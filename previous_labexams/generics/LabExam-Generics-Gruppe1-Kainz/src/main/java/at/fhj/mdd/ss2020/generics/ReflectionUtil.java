...
public class ReflectionUtil {
	...
	public static String getElementName(Field field) {
		if (!field.isAnnotationPresent(FieldMapping.class)) {
			throw new IllegalArgumentException("FieldMapping annotation missing from " + field.getName());
		}
	...
	public static String getElementName(Class<?> clazz) {
		if (!clazz.isAnnotationPresent(TelegramMapping.class)) {
			throw new IllegalArgumentException("TelegramMapping annotation missing from " + clazz.getName());
		}
